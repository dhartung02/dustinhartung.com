...
- Departs deliberately on color: the real product's actual Streamlit theme is a private-wealth / modern-fintech palette — deep near-black green canvas (`#0E1A14`), dark green panels (`#16241C`), muted warm gold accent (`#C9A86A`), soft near-white text (`#E7EBE6`). This case study's `CommandCenterFrame` and Command Center mockup use this real palette (translated to this portfolio's Tailwind conventions, not introduced as scattered raw hex values) as an authentic homage — the same treatment already given to Reqon's real teal/emerald brand accent. Sections outside the centerpiece (Hero, Case Study Overview, My Role, Outcome, Closing) stay within this portfolio's existing neutral dark theme so the page still reads as part of the same overall site.
...
- Section-card expand/collapse in `SectionGrid` uses a simple height/opacity transition (Framer Motion `AnimatePresence` or a CSS-only approach — implementer's choice, whichever is simpler given there's no nested state to preserve).
